...
NPZ = next(
    p
    for p in [
        DL / "best_screened/019_S_4549.npz",
        ROOT / "data/full_cache/019_S_4549.npz",
    ]
    if p.exists()
)
CAM = next(
    p
    for p in [
        DL / "best_screened/019_S_4549_t1_ad_logit.nii.gz",
        ROOT / "analysis_explainability_seed0_full/gradcam/019_S_4549_t1_ad_logit.nii.gz",
    ]
    if p.exists()
)
SUBJECTS = next(
